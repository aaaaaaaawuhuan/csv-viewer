...
- **简化列筛选界面**：读取文件后先不显示完整数据，而是先读取CSV表头，在界面左侧显示包含所有列的勾选框及"筛选"按钮，点击筛选按钮时才显示勾选的列。
...
#### 界面整合
- [x] 将筛选面板集成到主界面左侧
- [x] 调整主界面布局以容纳筛选面板和表格视图
- [x] 实现筛选面板与表格展示的联动
- [x] 进行功能测试和界面优化
- [x] 修复按钮点击区域问题

### 3.3 阶段三：层级树形控件开发（待开始）
...
- `setupFilterPanel()`: 设置左侧筛选面板，包含搜索输入框和按钮优化
- `populateColumnCheckboxes()`: 填充列勾选框列表
- `onFilterButtonClicked()`: 处理筛选按钮点击事件
- `filterCheckboxes(const QString &text)`: 根据搜索文本筛选显示列名勾选框
- `selectAllColumns()`: 全选所有列
- `clearAllColumns()`: 清空所有列的选择
